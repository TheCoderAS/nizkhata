...
/// The workbook is encrypted (legacy XOR/RC4 BIFF obfuscation). OOXML
/// EncryptedPackage containers are handled separately (office_crypto).
...
List<List<String>> decodeXls(Uint8List bytes) {
  ...
  try {
    ...
    throw XlsUnreadable(e.message);
  }
  if (cfb.hasStream('EncryptedPackage')) throw XlsPasswordProtected();
  final workbook = cfb.readStream('Workbook') ?? cfb.readStream('Book');
  if (workbook == null) {
